...
EFI_STATUS efi_main(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable)
{
    ...
    Print(u"TinkyWinkyOS loader v2024.08\r\n");

    EFI_LOADED_IMAGE *LdImg;
    BS->HandleProtocol(ImageHandle, &LoadedImageProtocol, &LdImg);

    EFI_FILE_HANDLE Root = LibOpenRoot(LdImg->DeviceHandle);

    Elf64_Ehdr *Ehdr = LdrLoadFile(Root, u"\\twos\\kernel", NULL);
    ...
    for (UINTN i = 0; i < Ehdr->e_phnum; i++) {
        ...
        if (Phdr->p_type == PT_LOAD) {
            Print(u"PT_LOAD offset=0x%lx filesz=0x%lx vaddr=0x%lx memsz=0x%lx flags=", Phdr->p_offset, Phdr->p_filesz,
            ...
            if (Phdr->p_vaddr & 0xfff) {
                Print(u"Error: segment base not aligned to page boundary\r\n");
                LdrExit();
            }
            ...
            for (UINTN Phys = Addr; Phys < PALIGN_UP(Addr + Phdr->p_memsz); Phys += EFI_PAGE_SIZE) {
                LdrMapPage(BootPML4, Phdr->p_vaddr + Phys - Addr, Phys, Flags);
            }
        }
    }
    ...
    for (UINTN i = 0; i < NoEnt; i++) {
        ...
        for (UINTN j = 0; j < Desc->NumberOfPages; j++) {
            LdrMapPage(BootPML4, VM_PHYS_MAP_START + Desc->PhysicalStart + j * EFI_PAGE_SIZE,
                       Desc->PhysicalStart + j * EFI_PAGE_SIZE, PG_WR);
        }
    }
    ...
    if (EFI_ERROR(BS->ExitBootServices(ImageHandle, MapKey))) {
        Print(u"ExitBootServices error\r\n");
        LdrExit();
    }
    ...
    if (EFI_ERROR(RT->SetVirtualAddressMap(NoEnt * DescSz, DescSz, DescVer, Map))) {
