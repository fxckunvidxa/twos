...
#define MALLOC_FAILURE_ACTION panic("dlmalloc failure")
...
#define dlrealloc_in_place __realloc_in_place
#define dlvalloc __valloc
#define dlpvalloc __pvalloc
#define dlmallinfo __mallinfo
...
static FORCEINLINE void *twos_mmap(size_t s)
{
  void *a = vm_mmap(VM_MR_KHEAP, NULL, NULL, PG_IDX_UP(s));
  return a ? a : MFAIL;
}

static FORCEINLINE int twos_munmap(void *a, size_t s)
{
  return vm_munmap(a, PG_IDX_UP(s)) ? 0 : -1;
}

#define MMAP_DEFAULT(s) twos_mmap(s)
#define MUNMAP_DEFAULT(a, s) twos_munmap((a), (s))
#define DIRECT_MMAP_DEFAULT(s) twos_mmap(s)
