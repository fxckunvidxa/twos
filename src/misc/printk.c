    ...
    while ((c = *format++) != '\0') {
        if (c == '%') {
            ...
            if (f == END_STRING) {
                ...
                uintval = (unsigned long long)(unsigned long)voidpval;
                n += do_conversion_int(uintval, f, width, precision, func, data);
                ...
                n += do_conversion_string("FORMAT ERROR", CONVERSION_CHARP, 0, 0, func,
                                          data);
            }
        } else {
            PUTCHAR(c);
...
int printk(const char *format, ...)
{
    ...
    int r;
    va_start(ap, format);
    bool state = vt_get_active_ctx()->cur_enabled;
    vt_set_cursor_state(vt_get_active_ctx(), false);
    r = vprintf(format, ap);
    vt_set_cursor_state(vt_get_active_ctx(), state);
    va_end(ap);
    return r;
}
...
int vsnprintf(char *str, usize size, const char *format, va_list ap)
{
